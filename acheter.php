...
<link rel='stylesheet' id='isotope-css'  href='wp-content/themes/css/acheter.css' type='text/css' media='all' />
...
<li id="nav-menu-item-422" class="page-scroll main-menu-item  menu-item-even menu-item-depth-0 menu-item menu-item-type-custom menu-item-object-custom"><a class="menu-link main-menu-link" href="boutique.php">Shop</a></li>
                ...
                <section id="features" class="section normal center" style="background-color:#232323; ">
                    <div class="container ">
                        <div class="wpb_wrapper"><h2 class="title ">Buy a product</h2>

                        <form action="acheter-produit.php" method="post">
                        <h4 class="text-acheter">Please select a product</h4>
<select class="dropdown" name="produit">
  <?php include("inc/config.php");
  $reponse = $bdd->query('SELECT * FROM produits ORDER BY ID DESC');
  while ($data = $reponse->fetch()) { ?> 
  <option class="select" value="<?php echo $data['nom']; ?> <?php echo $data['prix']; ?>"><?php echo $data['nom']; ?> <?php echo $data['prix']; ?></option>
  <?php } $reponse->closeCursor(); // Complete query ?>
</select> 
<div class="space-acheter"></div>
                        <h4 class="text-acheter">Name and forename that are on your PayPal</h4>
                        <input id="input-acheter" name="nom" type="text" required>
<div class="space-acheter"></div>
                        <h4 class="text-acheter">Email adress of your PayPal</h4>
                        <input id="input-acheter" name="email_paypal" type="email" required>
<div class="space-acheter"></div>
                        <h4 class="text-acheter">Discord Username (ex AAAAA#0000)</h4>
                        <input id="input-acheter" name="discord" type="text" required>
<div class="space-acheter"></div>
                        <h4 class="text-acheter"><b>Please send the exact amount of your product you have choose.</b></h4>
<div class="space-acheter"></div>
  <?php include("inc/config.php");
  $reponse = $bdd->query('SELECT * FROM config');
  while ($data = $reponse->fetch()) { ?> 
      <a href="<?php echo $data['lien_paypal_me']; ?>" target="_blank"><input type="btn" class="btn btn-success" value="Buy"></a>
  <?php } $reponse->closeCursor(); // Complete query ?>
<div class="space-acheter"></div>
                        <h4 class="text-acheter"><b>Clicking on the "Confirm" button will confirm that you have entered the correct information request, and that you have made the payment.</b></h4>
